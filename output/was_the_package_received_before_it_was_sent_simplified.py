"""Kata - Was the package received before it was sent? (Simplified)

completed at: 2025-07-30 13:56:09
by: Jakub Červinka
...
def was_package_received_yesterday(tz_from: int, tz_to: int, start: int, duration: int) -> bool:
    from_zone = timezone(timedelta(hours=tz_from))
    to_zone = timezone(timedelta(hours=tz_to))

    send_dt = datetime(2024, 1, 1, start, tzinfo=from_zone)
    arrival_dt = send_dt + timedelta(hours=duration)

    arrival_dt_local = arrival_dt.astimezone(to_zone)
    day_difference = (arrival_dt_local.date() - send_dt.date()).days
