...
def find_zombies(city):
    if not city or not city[0]:
        return []
    
    rows, cols = len(city), len(city[0])
    patient_zero_value = city[0][0]
    
    contaminated = [[0] * cols for _ in range(rows)]
    
    directions = [(-1, 0), (0, 1), (1, 0), (0, -1)]
    
    def dfs(row, col):
        if (row < 0 or row >= rows or col < 0 or col >= cols or 
            contaminated[row][col] == 1 or city[row][col] != patient_zero_value):
            return

        contaminated[row][col] = 1
